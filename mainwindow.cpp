...
void MainWindow::setupBooksTab()
{
    ...
    // Включаем перенос текста для заголовков, чтобы они полностью помещались
    booksTable->horizontalHeader()->setWordWrap(true);
    ...
    // Узкие колонки - фиксированные
        booksTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed); // Обложка
        booksTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Fixed); // Название - фиксированное
        booksTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Fixed); // Автор - фиксированное
        booksTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Fixed); // ISBN - фиксированное
        booksTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Fixed); // Год
        booksTable->horizontalHeader()->setSectionResizeMode(5, QHeaderView::Fixed); // Жанр
        booksTable->horizontalHeader()->setSectionResizeMode(6, QHeaderView::Fixed); // Доступна
        booksTable->horizontalHeader()->setSectionResizeMode(7, QHeaderView::Fixed); // Количество
        booksTable->horizontalHeader()->setSectionResizeMode(9, QHeaderView::Fixed); // Действия
...
void MainWindow::setupMembersTab()
{
    ...
    // Настраиваем растяжение колонок
        membersTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed); // Имя - фиксированная
        membersTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Fixed); // Фамилия - фиксированная
        membersTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Fixed); // Телефон - фиксированная
        membersTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Fixed); // Email - фиксированная
        membersTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Fixed); // Заблокирован
        membersTable->horizontalHeader()->setSectionResizeMode(6, QHeaderView::Fixed); // Действия
    ...
    // Панель фильтров для абонентов (все в одну строку)
...
void MainWindow::setupEmployeesTab()
{
    ...
    // Устанавливаем ширины колонок
    employeesTable->setColumnWidth(0, 160);  // Имя - шире
    employeesTable->setColumnWidth(1, 160);  // Фамилия - шире
    employeesTable->setColumnWidth(2, 140);  // Должность - шире для длинных названий
    employeesTable->setColumnWidth(3, 120);  // Зарплата - шире для больших чисел
    employeesTable->setColumnWidth(4, 110);  // Часы работы
    employeesTable->setColumnWidth(5, 140);  // Действия
    
    // Настраиваем растяжение колонок
        employeesTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Fixed); // Должность
        employeesTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Fixed); // Зарплата
        employeesTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Fixed); // Часы работы
        employeesTable->horizontalHeader()->setSectionResizeMode(5, QHeaderView::Fixed); // Действия
        // Имя и Фамилия - растягиваются
        employeesTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
...
void MainWindow::onShowOverdueBooks()
{
    try {
        ...
        if (!overdue.empty()) {
            ...
            // Настраиваем растяжение колонок
            overdueTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed);
            overdueTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Fixed);
            overdueTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Stretch); // Название книги растягивается
            overdueTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Fixed);
            overdueTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Fixed);
            overdueTable->horizontalHeader()->setSectionResizeMode(5, QHeaderView::Fixed);
